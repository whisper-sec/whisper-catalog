...
**Resolved: yes, which is exactly why flows are keyed and not keyless-tastable, while the single-request direct reads are.**
Every keyless request (success **or** a 400 refusal) decrements `x-ratelimit-limit: 100` (observed 96→93 across the boundary sweep, refusals included). A flow's **anchor** step is a single request and *can* be sampled keyless, but a full flow is dozens of `CALL`s (`indicator` ≈ 80 query-steps / doLength 98, `blast-radius` ≈ 55 / 67, `attack-surface` 30 / 33), and a single deep flow can exhaust the 100/window playground cap mid-run. **Conclusion:** the two-tier split follows the wire. The single-request direct read verbs are `access:"keyless"` / `playgroundTryable:true`: they run with no key as a rate-limited taste (100/window, real answers), and sending a key lifts the limit. Every multi-step flow and the write verb is `access:"keyed"` / `playgroundTryable:false`: a deep flow would exhaust the 100/window cap mid-run, and writes must be attributable. The README states the 100/window keyless cap plainly.

### 2. Cognition-verb playground exposure: do the read verbs run with no key?
**Resolved: empirically yes; the catalog marks them `access:"keyless"`.** `identify` and `db.schema()` were directly confirmed runnable keyless in the boundary map; the remaining direct verbs (`assess`, `variants`, `walk`, `explain`/`whisper.explain`, `psl.tldPlusOne`, `psl.affiliation`, `origins`, `history`, `history.whois`, `asSet`, `lookupTorRelay`) are all `isRead:true` and returned HTTP 200 valid shapes in the shape-probe pass. The boundary sweep established the empirical fact: **arbitrary reads (including deep multi-hop `MATCH` traversals and the catalog verbs) run keyless, bounded only by 100/window; no read was depth- or complexity-refused.** So each single-request direct read is `access:"keyless"` / `playgroundTryable:true`: it runs with no key (rate-limited 100/window, real answers), and a key is optional (it lifts the limit). Beyond these, Whisper has a second keyless surface outside this catalog, the non-Cypher identity ops (`verify`/`rdap`), which carry no key and no rate cap at all. (`asSet` returns a valid shape but 0 rows because membership data is unpopulated, not a permission block.)
...
**Rule applied (two-tier, follows the wire):** the single-request direct read verbs are **`access:"keyless"`** (rate-limited 100/window with no key, unlimited with a key); raw Cypher, every multi-step flow, and the write (`whisper.submit`) are **`access:"keyed"`** (X-API-Key). `access` and `playgroundTryable` agree: **`keyless`/`true`** for the single-request direct read verbs, **`keyed`/`false`** for the write and every multi-step flow (a deep flow would burn the 100/window keyless cap; writes must be attributable). Single-step reads have `exec.mode:direct` with the live Cypher + columns; multi-step read flows have `exec.mode:flow` (run via `run_workflow`). The `whisper.agents` control plane is **never** a catalog entry. Whisper's second keyless surface, the non-Cypher identity ops (`verify`/`rdap`), is not in this catalog.

**Totals: 29 entries, two-tier. 13 `access:"keyless"` / `playgroundTryable:true` (single-request direct reads), 16 `access:"keyed"` / `false` (15 multi-step flows + 1 write).**
